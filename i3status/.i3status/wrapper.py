...
def get_temp():
    ...
    try:
        cpu_temp = output[output.index("Package id 0: "):output.index("(high = +100.0")]
...
def get_ram_usage():
    ...
    ram = " " + str(usage) + "% (" + normalize_ram(used_m) + " / " + normalize_ram(tot_m) +  ") "
    color = COLOR[0]
...
if __name__ == '__main__':
    ...
    while True:
        ...
        j.insert(1, {'name' : 'ram', 'markup' : 'none', 'color' : r_color, 'full_text' : '%s' %ram})
        

        #TEMP
        (temp, t_color) = get_temp()
        j.insert(8, {'name' : 'temperatur', 'markup' : 'none', 'color' : t_color, 'full_text' : '%s' % temp})
